 ...
 * <pre>
 * class MyClass {
 *      private static final NKLog log = NKLog.get("MyClass");
 *
...
public class NKLog {
    ...
    private static String shortenTag(String tag) {
        if (tag.length() > 23) {
            ...
            Log.w("NKLog", "Log tag too long; shortening '" + tag + "' to '" + fixed + "'");
            return fixed;
        }
        return tag;
    }
    ...
    private NKLog(@NonNull String tag) {
        this.tag = shortenTag(tag);
    }

    @NonNull public static NKLog get(@NonNull String tag) {
        return new NKLog(tag);
    }
